...
class AttackerTree:
    ...
    def __init__(self, us_positive: bool = True) -> None:
        ...
        # Mirror the goal target onto the opponent's side. GOAL_POSITION is
        # defined assuming we attack +x; if we attack -x, negate the x.
        self.us_positive = us_positive
        self.goal_position: tuple[float, float] = (
            GOAL_POSITION if us_positive
            else (-GOAL_POSITION[0], GOAL_POSITION[1])
        )
